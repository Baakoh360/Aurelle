...
const ALL_DAILY_TIPS = [
  { icon: Droplets, text: 'Stay hydrated — aim for 8 glasses of water today.', colors: [Colors.tertiary, '#7DD3F0'] as [string, string] },
  { icon: Moon, text: 'Get 7–8 hours of sleep to support your cycle and mood.', colors: [Colors.secondary, '#B794F6'] as [string, string] },
  { icon: Sun, text: 'A few minutes of morning sunlight can boost your energy.', colors: [Colors.warning, '#FFD54F'] as [string, string] },
  { icon: Sparkles, text: 'Take a short walk or stretch to ease any tension.', colors: [Colors.primary, '#FF8FB3'] as [string, string] },
  { icon: Lightbulb, text: 'Eat a balanced breakfast to keep energy steady.', colors: ['#F59E0B', '#FBBF24'] as [string, string] },
  { icon: Droplets, text: 'Limit caffeine after 2 PM for better sleep.', colors: [Colors.tertiary, '#7DD3F0'] as [string, string] },
  { icon: Moon, text: 'Wind down with a book or calm music before bed.', colors: [Colors.secondary, '#B794F6'] as [string, string] },
  { icon: Sun, text: 'Spend time outdoors — even 10 minutes helps.', colors: [Colors.warning, '#FFD54F'] as [string, string] },
  { icon: Sparkles, text: 'Try 5 minutes of deep breathing or meditation.', colors: [Colors.primary, '#FF8FB3'] as [string, string] },
  { icon: Lightbulb, text: 'Add a vegetable or fruit to every meal today.', colors: ['#F59E0B', '#FBBF24'] as [string, string] },
  { icon: Droplets, text: 'Keep a water bottle nearby as a reminder.', colors: [Colors.tertiary, '#7DD3F0'] as [string, string] },
  { icon: Moon, text: 'Stick to a consistent bedtime tonight.', colors: [Colors.secondary, '#B794F6'] as [string, string] },
  { icon: Sun, text: 'Open the curtains or step outside in the morning.', colors: [Colors.warning, '#FFD54F'] as [string, string] },
  { icon: Sparkles, text: 'Do one thing that feels good for your body.', colors: [Colors.primary, '#FF8FB3'] as [string, string] },
  { icon: Lightbulb, text: 'Snack on nuts or fruit instead of processed options.', colors: ['#F59E0B', '#FBBF24'] as [string, string] },
  { icon: Droplets, text: 'Herbal tea counts toward your fluid intake.', colors: [Colors.tertiary, '#7DD3F0'] as [string, string] },
  { icon: Moon, text: 'Avoid screens 30 minutes before sleep.', colors: [Colors.secondary, '#B794F6'] as [string, string] },
  { icon: Sun, text: 'Take a break outside if you work indoors.', colors: [Colors.warning, '#FFD54F'] as [string, string] },
  { icon: Sparkles, text: 'Stretch your neck and shoulders if you sit a lot.', colors: [Colors.primary, '#FF8FB3'] as [string, string] },
  { icon: Lightbulb, text: 'Eat slowly and notice how your body feels.', colors: ['#F59E0B', '#FBBF24'] as [string, string] },
  { icon: Droplets, text: 'Start the day with a full glass of water.', colors: [Colors.tertiary, '#7DD3F0'] as [string, string] },
  { icon: Moon, text: 'Dim the lights in the evening to support sleep.', colors: [Colors.secondary, '#B794F6'] as [string, string] },
  { icon: Sun, text: 'Get natural light in the first hour after waking.', colors: [Colors.warning, '#FFD54F'] as [string, string] },
  { icon: Sparkles, text: 'Do one small act of kindness for yourself.', colors: [Colors.primary, '#FF8FB3'] as [string, string] },
];

/** Seed from date (varies by day) */
...
/** Session seed: different every time the component mounts (e.g. app restart), so tips change on reopen */
function useSessionSeed() {
  const ref = useRef<number | null>(null);
  if (ref.current === null) ref.current = getDaySeed(new Date()) * 1000 + (Date.now() % 100000);
  return ref.current;
}

/** Shuffle array deterministically by seed (same seed = same order) */
function shuffleBySeed<T>(array: T[], seed: number): T[] {
  const out = [...array];
...
export default function HomeScreen() {
  ...
  const todayTip = (() => {
    if (!cycleData) return TIPS_SAFE[sessionSeed % TIPS_SAFE.length];
    const phase = getPhaseForDate(today);
    let bank: string[];
    if (phase === 'period') bank = TIPS_PERIOD;
    else if (phase === 'fertile') bank = TIPS_FERTILE;
    else if (phase === 'ovulation') bank = TIPS_OVULATION;
    else bank = TIPS_SAFE;
    return bank[sessionSeed % bank.length];
  })();

  const dailyTipsShuffled = shuffleBySeed(ALL_DAILY_TIPS, sessionSeed).slice(0, 4);
      ...
      >
        ...
        <View style={styles.section}>
          ...
          {dailyTipsShuffled.map((item, index) => {
            ...
            const key = `${sessionSeed}-${index}-${item.text.slice(0, 12)}`;
            ...
            return (
              ...
              >
                <View style={[styles.tipRowIconWrapCard, { backgroundColor: item.colors[0] + '50' }]}>
                  <Icon size={22} color={item.colors[0]} />
                </View>
